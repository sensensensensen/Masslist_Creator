...
function task_assign_formulas()
    ...
    try
        ...
        # Iterate to find conflicts
        i = 1
        while i < nrow(results)
            # Find cluster of peaks within window
            cluster_idxs = [i]
            j = i + 1
            ...
            if length(cluster_idxs) > 1
                # Conflict found! Decide winner
                # Priority 1: Has Formula vs Unknown
                has_formula = [results.AssignedFormula[idx] != "?" for idx in cluster_idxs]
                
                if any(has_formula)
                    # Keep only those with formula
                    # If multiple have formula, keep highest intensity among them
                    valid_idxs = cluster_idxs[has_formula]
                    best_idx = valid_idxs[argmax(results.Intensity[valid_idxs])]
                else
                    # All unknown, keep highest intensity
        ...
        # Re-generate labels_to_plot for graph based on cleaned results

...
This module contains the core logic for generating chemical formula candidates, calculating isotopic patterns, fitting peaks,
    and assigning formulas to create a mass list for Toftracer2 or other mass spectrum analysis tools based on Mickwitz et al. 2025 AMT paper.
# Key Features
- **Formula Generation**: Generates a comprehensive list of chemical formulas based on user-defined rules and constraints.  
- **Isotopic Pattern Calculation**: Computes the expected isotopic distribution for each formula candidate, which can be used for more accurate peak assignment.
- **Peak Fitting**: Fits detected peaks in the mass spectrum to theoretical profiles, allowing for precise determination of peak centers and intensities.
- **Formula Assignment**: Matches fitted peaks to the generated formula candidates based on mass accuracy and isotopic pattern similarity, producing a final mass list for export.
# !!! Note on Usage!!!!
...
# Arguments
- `FormulaConfig`: A struct that holds user-defined parameters for formula generation, including atomic count limits and ratio constraints.
- `ATOMIC_MASS`: A dictionary containing the monoisotopic masses of relevant elements and their isotopes.
- `ION_MODES`: A dictionary defining common adducts for different ionization modes, including their mass shifts and priorities for formula assignment.
- `BACKGROUND_TARGETS`: A dictionary of common background ions for each ionization mode, which can be used for annotation or subtraction.
# Functions
- `parse_formula(f::AbstractString)`: Parses a chemical formula string and returns a dictionary mapping element symbols to their respective atom counts.
- `calculate_neutral_mass(f_input)`: Calculates the neutral mass of a chemical formula given as a string or a tuple containing the formula string.
...
module ChemUtils
    ...
    # Central control for formula generation rules. Users can adjust these parameters to tailor the formula generation process without modifying the core logic.
    ...
    const USER_CONFIG = FormulaConfig()
    # =================  !!!!!! USER CONFIGURATION END !!!!!! =================
    
    # Isotopic Abundances (For pattern calculation)
    ...
    # [EXPORT HELPERS] Background Ion Decomposition (For potential background subtraction or annotation)
...
"""
parse_formula(f::AbstractString) -> Dict{String, Int}
Parse a chemical formula string and return a dictionary mapping element symbols to atom counts.
# Arguments
- `f::AbstractString`: The chemical formula as a string (e.g., "C6H12O6", "C(13)H(12)O(6)", "C5H5N(15)").
# Returns
- `Dict{String, Int}`: A dictionary where keys are element symbols (e.g., "C", "H", "N", "O", "C13", "N15") and values are the corresponding atom counts.
# Notes
- The function handles both standard and isotopically labeled formulas, as well as formulas with parentheses for isotopes.
- If the formula string is empty or invalid, it returns an empty dictionary.
"""   
...
"""
calculate_neutral_mass(f_input) -> Float64
Calculate the neutral mass of a chemical formula given as a string or a tuple containing the formula string.
# Arguments
- `f_input`: The chemical formula, which can be either a string (e.g., "C6H12O6") or a tuple where the first element is the formula string (e.g., ("C6H12O6",)).
# Returns
- `Float64`: The calculated neutral mass of the formula based on the atomic composition and monoisotopic masses of the elements.
"""    
    ...
    # [MODIFIED] Using Configuration Struct
    function generate_formula_db(min_m, max_m; cfg::FormulaConfig = USER_CONFIG)
        db = FormulaCandidate[]
        mC, mH, mN, mO, mS, mSi = ATOMIC_MASS["C"], ATOMIC_MASS["H"], ATOMIC_MASS["N"], ATOMIC_MASS["O"], ATOMIC_MASS["S"], ATOMIC_MASS["Si"]
        
        # 1. Carbon Loop (Using Config)
        for nC in 0:cfg.max_C 
            mass_C = nC * mC
            if mass_C > max_m; break; end
            
            # 2. Hydrogen Loop (Using Config)
            for nH in 0:cfg.max_H
                mass_CH = mass_C + nH * mH
                if mass_CH > max_m; break; end
                
                # [FILTER] Empty Molecule
                if nC == 0 && nH == 0; 
                    # pass 
                end 

                # [FILTER] Organic Rules (C >= 1)
                if nC > 0
                    hc = nH / nC
                    if !(cfg.org_min_HC <= hc <= cfg.org_max_HC); continue; end
                end
                
                # [FILTER] Inorganic Rules (C = 0)
                if nC == 0
                    if nH > cfg.inorg_max_H; continue; end
                end

                # 3. Nitrogen Loop (Using Config)
                for nN in 0:cfg.max_N
                    mass_CHN = mass_CH + nN * mN
                    if mass_CHN > max_m; break; end
                    
                    if nC > 0
                        # [FILTER] Valence / DBE Check
                        dbe = nC - 0.5*nH + 0.5*nN + 1
                        if dbe < 0; continue; end # Must be physically possible
                        if dbe > cfg.max_DBE; continue; end # [NEW] Max DBE limit
                        
                        if cfg.filter_radicals
                            if dbe % 1 != 0; continue; end # [NEW] Filter radicals if enabled
                        end

                        # [FILTER] N/C Ratio
                        if !(0.0 <= nN/nC <= cfg.org_max_NC); continue; end
                    end

                    # 4. Oxygen Loop (Using Config)
                    for nO in 0:cfg.max_O
                        mass_CHNO = mass_CHN + nO * mO
                        if mass_CHNO > max_m; break; end
                        
                        if nC > 0
                            # [FILTER] O/C Ratio
                            if !(0.0 <= nO / nC <= cfg.org_max_OC); continue; end
                        end

                        # 5. Sulfur Loop (Using Config)
                        for nS in 0:cfg.max_S
                            mass_CHNOS = mass_CHNO + nS * mS
                            if mass_CHNOS > max_m; break; end
                            
                            # [FILTER] Inorganic Heteroatom Check
                            if nC == 0
                                if cfg.inorg_must_have_S
                                    if nS == 0; continue; end # Must contain S (Filters HNO3)
                                else
                                    if nS == 0 && nN == 0; continue; end # Default fallback
                                end
                            end
                            
                            # 6. Silicon Loop (Using Config)
                            for nSi in 0:cfg.max_Si
...
"""
calculate_isotopes(formula_comp::Dict{String, Int}) -> Vector{Tuple{Float64, Float64}}
Calculate the isotopic pattern for a given chemical formula composition. Returns a vector of tuples containing mass shifts and relative abundances normalized to the base peak.
# Arguments
- `formula_comp::Dict{String, Int}`: A dictionary representing the chemical formula composition, where keys are element symbols (e.g., "C", "H", "N", "O") and values are the corresponding atom counts.
# Returns
- `Vector{Tuple{Float64, Float64}}`: A vector of tuples, where each tuple contains:
    - `Float64`: The mass shift from the monoisotopic peak (in Da).
    - `Float64`: The relative abundance of that isotopic peak, normalized to the base peak (the most abundant isotopic variant).
"""    
...
# ==============================================================================
"""
AppState struct holds the entire state of the application, including loaded data, user parameters, and observables for reactive plotting.
"""
...
"""
get_peak_model(mz_center) -> Function
Returns a peak shape model function based on the provided m/z center. 
    If user-defined peak shapes are available, it uses interpolation to create a custom model. 
    Otherwise, it defaults to a Gaussian profile with a fixed resolution of 10,000.
# Arguments
- `mz_center`: The m/z value around which the peak model should be centered. This value is used to determine the appropriate peak shape, either by interpolation of user-provided data or by calculating the Gaussian width based on a fixed resolution.
# Returns
- `Function`: A function that takes two arguments, `x` (the m/z values to evaluate) and `p` (a parameter vector where `p[1]` is the peak intensity and `p[2]` is the peak center), and returns the modeled peak shape values at the given `x` positions.
"""
...
"""
Fit peaks in a chunk of mass spectrum data for every Da.
# Arguments
- `mz_chunk`: A vector of m/z values for the current chunk of data.
- `i_chunk`: A vector of intensity values corresponding to the m/z values in `mz
_chunk`.
- `known_formulas`: A DataFrame containing known chemical formulas and their properties, used for fitting known peaks.
- `ion_mode`: A string indicating the ionization mode (e.g., "H+", "NH4+"), which determines the adducts to consider during fitting.
- `threshold`: A float value representing the minimum intensity required for a peak to be considered valid during fitting.
# Returns
- `peaks_found`: A DataFrame containing the fitted peaks found in the chunk, including their m/z values, intensities, types (known or unknown), and reference formulas if applicable.       
- `fitted_signal`: A vector representing the fitted signal across the m/z chunk, which can be used for plotting or further analysis.
"""
...
"""
Main task to fit peaks in the mass spectrum data.
    This function processes the mass spectrum data in chunks, fits peaks using both known formulas and unknown detections, 
    and updates the application state with the results for plotting and formula assignment.
"""
...
"""
Main task to assign chemical formulas to the fitted peaks.
    This function first attempts to match peaks to known formulas, then checks for background ion matches, and finally performs a database search for unknown peaks. 
    It also includes a post-processing step to clean up closely spaced peaks that may represent the same ion (de-splitting).
"""
...
# ==============================================================================
"""
Main function to construct the GUI layout, initialize widgets, and bind them to the application state and core functions.
    This function sets up the sidebar with controls for data loading, parameter adjustments, and analysis actions, 
        as well as the main plot area for visualizing the spectrum and fitted peaks. 
    It also includes a log area for status updates and a progress slider for long-running tasks.
"""
function main()

...
mutable struct AppState
    ...
    function AppState()
        new("", [], [], Float64[], Matrix{Float64}(undef, 0,0), Float64[],
            ...
            # [FIXED] Default range set to 17.0 - 400.0
...
function task_fit_peaks()
    try
        ...
        # [FIXED] Force sync reading from GUI State to Internal State
        # This ensures user input (e.g. 50-350) is respected immediately
        # (Actually handled in btn_fit but good to rely on Observable here)
...
function main()
    ...
    # [FIXED] Default strings set to 17 and 400
    ...
    on(btn_fit.clicks) do _
        # [FIXED] Force update params from GUI

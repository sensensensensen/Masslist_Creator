...
module ChemUtils
    ...
    const ISOTOPE_ABUNDANCE = Dict(
        ...
        "Si"=> [("Si", 0.922), ("Si29", 0.047), ("Si30", 0.031)] # Simplified
    )
    ...
    # [EXPORT HELPERS] Offsets for Decomposition (Neutral part added to M)
    # H+ column is always 1 separately.
    ...
    # Background ion decomposition (Neutral Core)
...
function main()
    ...
    on(btn_export.clicks) do _
        ...
        if !isempty(path)
            ...
            open(path, "w") do io
                # 1. Header Metadata (Tofware Format)
                println(io, "# Elements:")
                ...
                for row in eachrow(STATE.assignment_table)
                    ...
                    if row.AssignedFormula == "?" || ismissing(row.AssignedFormula)
                        # Unknown: Keep 0s, Name empty
                        name_str = ""
                    else
                        # Assigned: Decompose
                        # A. Get Adduct Offsets
                        ...
                        if row.AdductType == "Background"
                            ...
                            # Sum Base + Adduct
                            # Only handle C, H, N, O, S, Si for now in offsets
                            ...
                            # Isotopes (usually not in simple formula strings but handled by parser if present)
                            c13 = get(base_atoms, "C13", 0)
                            o18 = get(base_atoms, "O18", 0)
                            s34 = get(base_atoms, "S34", 0)
                            
                            h_plus = 1 # All our supported adducts are singly charged positive
                            name_str = "$(row.AssignedFormula) $(row.AdductType)"
                        end
                    end
                    
                    # Write Row (Tab Separated)
                    println(io, "$c\t$c13\t$h\t$h_plus\t$n\t$o\t$o18\t$s\t$s34\t$i_el\t$si\t$(row.Mass)\t$name_str")
                end
            end

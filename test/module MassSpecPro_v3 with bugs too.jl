...
mutable struct AppState
    ...
    # [NEW] Peak Locations for Vertical Lines
    ...
    function AppState()
        # [FIX] Initialize obs_peak_locs with [NaN] instead of empty array []
        # to prevent "reducing over an empty collection" error in vlines! autolimits
        new("", [], [], Float64[], Matrix{Float64}(undef, 0,0), Float64[],
            DataFrame(), [], DataFrame(), DataFrame(),
            Observable(Float64[]), Observable(Float64[]),
            Observable(Float64[]), Observable(Float64[]),
            Observable(Float64[]),
            Observable([NaN]), # obs_peak_locs (Fixed)
...
function task_fit_peaks()
    try
        ...
        # [FIX] Reset peak locations to [NaN] (invisible) instead of empty to prevent crash
        ...
        # [FIX] Update Peak Locations with fallback to [NaN]
    ...
    try
            ...
            for adduct in adduct_list
                ...
                if r_start <= r_end && r_start <= db_len
                    ...
                    if best_local_match !== nothing
                        ...
                        push!(labels_to_plot, (row.Mass, row.Intensity, best_local_match.formula_str))
                        match_found_for_peak = true
                        break 
                    end
                end
            end
            if !match_found_for_peak
        ...
        if !isempty(labels_to_plot)
            ...
            for i in 2:length(labels_to_plot)
                ...
                # Overlap Condition: Same Formula AND Close Mass (< 0.15 Da)
                if curr[3] == prev[3] && abs(curr[1] - prev[1]) < 0.15
                    # Keep the one with higher intensity
                    if curr[2] > prev[2]
                        unique_labels[end] = curr
                    end
                else
...
function main()
    ...
    # Vertical lines for fitted peaks (Green, Dashed)
    # Observable is now initialized with [NaN] to prevent autoscaling crash on empty data
